...
elseif($reqForm== "skcpns")
{
	...
	$set_upload->selectByParamsUpload(array("A.SK_CPNS_FILE_ID"=>$reqFileId),-1,-1,$statement);
}
...
elseif($reqForm== "upload_multi")
{
	$set_upload= new UploadFile();
	$set_upload->selectByParams(array("A.UPLOAD_FILE_ID"=>$reqFileId),-1,-1,$statement);
	$link="LINK_FILE";
	$namafile="NAMA_FILE";
}
else
...
if(!empty($tempLinkFile))
{
	if($image==1)
	{
		$imageData = base64_encode(file_get_contents($tempLinkFile));

		$src = 'data: '.mime_content_type($tempLinkFile).';base64,'.$imageData;
		echo '<img src="' . $src . '">';
