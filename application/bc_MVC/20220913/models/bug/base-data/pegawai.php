<? 
  ...
  class Pegawai extends Entity{ 
    ...
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="")
	{
		...
		, P.*
		FROM simpeg.pegawai P
		LEFT JOIN simpeg.pangkat A1 ON A1.PANGKAT_ID = P.LAST_PANGKAT_ID
		WHERE 1=1
		"; 
    ...
    function updateuploadfoto()
	{
		$str = "
				UPDATE PEGAWAI_FILE
				SET    
					   PEGAWAI_FILE_ID= ".$this->getField("PEGAWAI_FILE_ID").",
					   NAMA_FILE_FOTO= '".$this->getField("NAMA_FILE_FOTO")."',
					   LINK_FILE_FOTO= '".$this->getField("LINK_FILE_FOTO")."',
					   TIPE_FILE_FOTO= '".$this->getField("TIPE_FILE_FOTO")."',
    ...
    function getCountByUmur($paramsArray=array(), $statement = '')
	{			   
		$str = "
				SELECT COUNT(1) JML
				FROM(
					SELECT  A.PEGAWAI_ID, Y.USIA_TAHUN
					FROM PEGAWAI A					 
					INNER JOIN (
					SELECT PEGAWAI_ID,
					CASE WHEN (TO_NUMBER_FILTER(TO_CHAR(CURRENT_DATE, 'MM'))-TO_NUMBER_FILTER(TO_CHAR(TANGGAL_LAHIR, 'MM')))<0 THEN 
					   TO_NUMBER_FILTER(TO_CHAR(CURRENT_DATE, 'YYYY'))-TO_NUMBER_FILTER(TO_CHAR(TANGGAL_LAHIR, 'YYYY'))-1
					   ELSE TO_NUMBER_FILTER(TO_CHAR(CURRENT_DATE, 'YYYY'))-TO_NUMBER_FILTER(TO_CHAR(TANGGAL_LAHIR, 'YYYY')) END USIA_TAHUN
					FROM PEGAWAI
					) Y ON A.PEGAWAI_ID = Y.PEGAWAI_ID
					INNER JOIN SATKER C ON A.SATKER_ID = C.SATKER_ID 
					WHERE 1=1 
					 AND (STATUS_PEGAWAI = 1 OR STATUS_PEGAWAI = 2 OR STATUS_PEGAWAI = 5)
					 ".$statement."
					 ORDER BY Y.USIA_TAHUN
					 ) A
                WHERE 1=1
				";
    ...
    function selectByParamsJenisKelaminPns($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder=" ORDER BY NAMA")
	{
		...
		( 
			...
			INNER JOIN STATUS_PEGAWAI B ON A.STATUS_PEGAWAI = B.STATUS_PEGAWAI_ID AND A.STATUS_PEGAWAI IN (2)
    ...
    function selectByStatistik($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder="ORDER BY GOL_RUANG")
	{
		$str = "
				SELECT COUNT(1) JML, COALESCE(GOL_RUANG, 'Data Tidak terdefinsi') GOL_RUANG
                FROM(
                    SELECT  A.PEGAWAI_ID, B.GOL_RUANG
                    FROM PEGAWAI A
                    LEFT JOIN PANGKAT_TERAKHIR B ON A.PEGAWAI_ID = B.PEGAWAI_ID 
					INNER JOIN SATKER C ON A.SATKER_ID = C.SATKER_ID                   
                    WHERE 1=1
					 ".$statement." ORDER BY B.GOL_RUANG
				) A
				WHERE 1=1
				"; 
		while(list($key,$val) = each($paramsArray))
		...
		$str .= " GROUP BY GOL_RUANG ".$sOrder;
		$this->query = $str;
		
		return $this->selectLimit($str,$limit,$from); 
    }

    function selectByParamsJumlahPns($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder=" ORDER BY A.TIPE_PEGAWAI_NEW_ID_PARENT")
	{
		$str = "
		SELECT A.TIPE_PEGAWAI_NEW_ID_PARENT, COALESCE(NAMA, 'Data Tidak terdefinsi') NAMA, COALESCE(B.JUMLAH,0) JUMLAH
		FROM
		(
			SELECT '1' TIPE_PEGAWAI_NEW_ID_PARENT, 'Struktural' NAMA
			UNION ALL
			SELECT '2' TIPE_PEGAWAI_NEW_ID_PARENT, 'Fungsional' NAMA
			UNION ALL
			SELECT '3' TIPE_PEGAWAI_NEW_ID_PARENT, 'Pelaksana' NAMA
		) A
		LEFT JOIN
		(
			SELECT COUNT(1) JUMLAH,A.TIPE_PEGAWAI_NEW_ID_PARENT
			FROM
			(
				SELECT C.NAMA NAMA_TIPE,C.TIPE_PEGAWAI_NEW_ID_PARENT
				FROM PEGAWAI A
				INNER JOIN 
				(
					 SELECT A.PEGAWAI_JABATAN_ID,A.TIPE_PEGAWAI_NEW_ID,B.TIPE_PEGAWAI_NEW_ID_PARENT,B.NAMA
					 FROM PEGAWAI_JABATAN A
					 INNER JOIN TIPE_PEGAWAI_NEW B ON B.TIPE_PEGAWAI_NEW_ID = A.TIPE_PEGAWAI_NEW_ID
				)C ON A.PEGAWAI_JABATAN_ID = C.PEGAWAI_JABATAN_ID
				WHERE C.TIPE_PEGAWAI_NEW_ID_PARENT IN ('1','2','3') 
				AND (STATUS_PEGAWAI = 1 OR STATUS_PEGAWAI = 2 OR STATUS_PEGAWAI = 4 OR STATUS_PEGAWAI = 5)  
			) A
			GROUP BY A.TIPE_PEGAWAI_NEW_ID_PARENT

		) B ON A.TIPE_PEGAWAI_NEW_ID_PARENT = B.TIPE_PEGAWAI_NEW_ID_PARENT
		"; 
    ...
    function selectByParamsServer($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder=" ORDER BY LINK_SERVER_ID")
	{
     ...
     function selectByParamsUpload($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder=" ORDER BY A.PEGAWAI_ID")
	{
		$str = "
		SELECT A.PEGAWAI_FILE_ID, A.PEGAWAI_ID, 
		A.NAMA_FILE_FOTO ,
		A.LINK_FILE_FOTO ,
		A.NAMA_FILE_KARPEG ,
		A.LINK_FILE_KARPEG ,
