...
class kenaikan_gaji_berkala_guru_json extends CI_Controller {
	...
	function json_pegawai()
	{
		...
		while ($set->nextRow()) 
		{
			...
			foreach($columnsDefault as $valkey => $valitem) 
			{
				...
				{
					if($set->getField("STATUS_KGB")== '3')
					{
						$row[$valkey] = $set->getField($aColumns[$i]);
					}
					else
					{
						$row[$valkey] = "<input type='checkbox' class='editor-active' id='reqPilihCheck".$no_urut."' value='".$set->getField("PEGAWAI_ID")."-".$set->getField("STATUS_KGB")."-".$set->getField("SATKER_ID")."'>";
					}
				}
