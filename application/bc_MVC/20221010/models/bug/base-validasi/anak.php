  ...
  class Anak extends Entity{ 
    ...
    function insert()
	{
		...
		$str = "INSERT INTO validasi.ANAK (
				   ...
				   TANGGAL_UPDATE, FOTO, USER_APP_ID, LAST_CREATE_USER, LAST_CREATE_DATE, LAST_CREATE_SATKER, TEMP_VALIDASI_ID)
				VALUES (
    ...
    function insertupload()
	{
		...
		$str = "INSERT INTO ANAK_FILE(
	            ANAK_FILE_ID, ANAK_ID, PEGAWAI_ID, TEMP_VALIDASI_ID, 
	...
	{
		$str = "
		SELECT A.ANAK_ID, A.PEGAWAI_ID, TEMP_VALIDASI_HAPUS_ID, A.PENDIDIKAN_ID, 
			...
			,COALESCE(PJ.LINK_FILE,PA.LINK_FILE) LINK_FILE,COALESCE(PJ.ANAK_FILE_ID,PA.ANAK_FILE_ID)ANAK_FILE_ID,COALESCE(PJ.LINK_SERVER,PA.LINK_SERVER)LINK_SERVER

		FROM validasi.validasi_pegawai_anak A
		LEFT JOIN PENDIDIKAN B ON A.PENDIDIKAN_ID = B.PENDIDIKAN_ID
		LEFT JOIN ANAK_FILE PJ ON PJ.TEMP_VALIDASI_ID = A.TEMP_VALIDASI_ID
		LEFT JOIN ANAK_FILE PA ON PA.ANAK_ID = A.ANAK_ID
		WHERE 1 = 1
		"; 
	...
	{
		$str = "
		SELECT A.ANAK_FILE_ID, A.ANAK_ID, A.PEGAWAI_ID, A.TEMP_VALIDASI_ID, 
		A.NAMA_FILE, A.LINK_FILE, A.TIPE_FILE,A.LINK_SERVER
		FROM ANAK_FILE A
		LEFT JOIN validasi.ANAK B ON A.TEMP_VALIDASI_ID = B.TEMP_VALIDASI_ID
		WHERE 1=1
		"; 
