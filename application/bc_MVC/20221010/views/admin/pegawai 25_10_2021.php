...
$this->load->model("base-app/TipePegawai");
$this->load->model("base-validasi/TipePegawai");


$reqId= $this->input->get('reqId');

$statement_tipe= " AND ( TIPE_PEGAWAI_ID_PARENT = '0' OR LENGTH(TIPE_PEGAWAI_ID) = 2)";
$tipe_pegawai= new TipePegawai();
$tipe_pegawai->selectByParams(array(), -1,-1, $statement_tipe);
                    ...
                    <div class="row mb-8">
                        ...
                        <div class="col-lg-3 mb-lg-0 mb-4">
                            ...
                            <select name="reqTipePegawai" id="reqTipePegawai" class="form-control datatable-input">
                                ...
                                {
                                    ?>
                                    <option value="<?=$tipe_pegawai->getField("TIPE_PEGAWAI_ID")?>"><?=$tipe_pegawai->getField("NAMA")?></option>
                                    <?
                                }
                                ?>
                            </select>
...
jQuery(document).ready(function() {
    $("#reqSatuanKerja,#reqStatusPegawai,#reqBulan,#reqTipePegawai").change(function() {
      ...
      jsonurl= "json-admin/pegawai_json/json?reqTipePegawai="+reqTipePegawai+"&reqStatusPegawai="+reqStatusPegawai+"&reqSatuanKerja="+satuanKerja;
      // datanewtable.DataTable().filter.reqValidasi = 1;
      // datanewtable.DataTable().draw();
      datanewtable.DataTable().ajax.url(jsonurl).load();
    });

    var jsonurl= "json-admin/pegawai_json/json?reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>&reqValidasi=<?=$reqValidasi?>&reqSatuanKerja=<?=$reqSatuanKerja?>";
