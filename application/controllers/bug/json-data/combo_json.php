...
class combo_json extends CI_Controller {
	...
	function autocompletejabatannew()
	{
		...
		// print_r($reqKategoriJabatanParentId);exit;
	...
	function autocompletedatapegawai()
	{
		...
		{
			$statement.= " AND A.NIP_BARU = '".$search_term."'";
		}
		else
		{
			$statement.= " AND UPPER(A.NAMA) LIKE '%".strtoupper($search_term)."%' ";
		...
		$jumlahdata= $set->getCountByParams();
		$set->selectByParamsDataCombo(array(), $limit, $from, $statement, $sorder);
		// echo $set->query;exit;
		while($set->nextRow())
		{
			$arrdata= [];
			$arrdata["id"]= $set->getField("PEGAWAI_ID");
			$arrdata["text"]= $set->getField("PEGAWAI_INFO");
			$arrdata["description"]= $set->getField("PEGAWAI_INFO");
			$arrdata["nip"]= $set->getField("NIP_BARU");
			$arrdata["nama"]= $set->getField("NAMA");
			$arrdata["jabatan"]= $set->getField("JABATAN");
			$arrdata["golongan"]= $set->getField("GOL_RUANG");
			$arrdata["unor"]= $set->getField("SATKER_INDUK_NAMA");
			$arrdata["tmt_gol"]= dateToPageCheck($set->getField("TMT_PANGKAT"));
			array_push($arrdetildata, $arrdata);
		}
