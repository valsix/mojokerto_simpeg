...
class api_kompetensi_json extends REST_Controller {
    ...
    function index_get() {
        ...
        if($reqToken == '')
        {
            ...
            while($set->nextRow())
            {
                ...
                {  
                    if($setPenilaian->getField('aspek_id')=='2'){
                        $result[$total]['kompetensi'][$totalPenilaian]['categori'] = $setPenilaian->getField('nama');
                        $result[$total]['kompetensi'][$totalPenilaian]['nilai'] = $setPenilaian->getField('nilai');
                        $result[$total]['kompetensi'][$totalPenilaian]['nilai_standart'] = $setPenilaian->getField('nilai_standar');
                    $totalPenilaian++;
                    }
                }
