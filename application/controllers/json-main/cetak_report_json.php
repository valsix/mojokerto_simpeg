...
class cetak_report_json extends CI_Controller {
	...
	function eselon_jabatan()
	{
		...
		$objPHPexcel= PHPExcel_IOFactory::load('Templates/report/eselon_jabatan.xlsx');
		$style = array(
			'borders' => array(
				'allborders' => array(
		...
		$field= array("NO", "NAMA", "TOT21", "TOT22", "TOT31", "TOT32", "TOT41", "TOT42", "TOT51","TOT52","TOT");
		...
		$this->load->model("base/Rekap");
		$set = new Rekap();
		$set->selectByParamsBkppRekapEselonJabatan(array(), -1, -1, $statement, "");	
		// $urut=1;
		$nomor=1;
		$kolomawal=1;
		$row = 10;
		...
		while($set->nextRow()){	
			...
			for($i=0; $i<count($field); $i++)
			{
				...
				else if($field[$i] == "NAMA" ||  $field[$i] == "NAMA_JABATAN" )
				{
					$objWorksheet->setCellValue($kolom.$row,$set->getField($field[$i]));
				}
				else if($field[$i] == "JUMLAH" )
		...
		$objWorksheet->mergeCells('A'.$row.':'.'B'.$row);
		$objWorksheet->setCellValue("A".$row,"JUMLAH TOTAL JABATAN STRUKTURAL");
		$objWorksheet->getStyle("A".$row)->getFont()->setBold( true );
		$objWorksheet->getStyle("A".$row)->applyFromArray($styleTengahC);
		...
		$objWorksheet->mergeCells('A'.$rowtotal.':'.'B'.$rowtotal);
		$objWorksheet->setCellValue("A".$rowtotal,"JUMLAH JABATAN STRUKTURAL TERISI");
		$objWorksheet->getStyle("A".$rowtotal)->getFont()->setBold( true );
		$objWorksheet->getStyle("A".$rowtotal)->applyFromArray($styleTengahC);
		...
		for($i=2; $i<=$index_kolom-1; $i++)
		{
			// print_r($kolom);
			$kolom= toAlpha($i);
			$rowAwal=setToAlpha($i, $row); $rowAkhir=setToAlpha($i, $rowtotal+1);
			// print_r($kolom.$row-1."/"."=SUM(".$rowAwal.":".$rowAkhir.")");
			$objWorksheet->setCellValue($kolom.$rowtotal,"=".$rowAwal."-".$rowAkhir."");
			$objWorksheet->getStyle($kolom.$rowtotal)->applyFromArray($styleTengahC);
		}
		...
		$objWorksheet->mergeCells('A'.$rowtotal.':'.'B'.$rowtotal);
		$objWorksheet->setCellValue("A".$rowtotal,"JUMLAH JABATAN STRUKTURAL KOSONG");
		$objWorksheet->getStyle("A".$rowtotal)->getFont()->setBold( true );
		$objWorksheet->getStyle("A".$rowtotal)->applyFromArray($styleTengahC);
		

		$arrayData=array("ESELON21", "ESELON22", "ESELON31", "ESELON32", "ESELON41", "ESELON42", "ESELON51", "ESELON52");


		$setnew = new Rekap();
		$setnew->selectByParamsBkppTotalRekapEselonJabatanKosong(array(), -1, -1, $statement, "");	
		// $urut=1;
		$rownew = $rowtotal;

		// print_r($rownew);exit;
		// echo $set->query; exit; 
		while($setnew->nextRow()){	
			...
			for($i=0; $i<count($arrayData); $i++)
			{
				...
				$kolomnew= toAlpha($index_kolom_new);	
				// print_r($kolom);				
				$objWorksheet->setCellValue($kolomnew.$rownew,$setnew->getField($arrayData[$i]));
				$objWorksheet->getStyle($kolomnew.$rownew)->applyFromArray($styleT);
		...
		$rowkosong=$rownew-1;
		$kolomkosong= toAlpha($index_kolom_new);
		$rowAwal=setToAlpha(2, $rowkosong); $rowAkhir=setToAlpha(9, $rowkosong);
		$objWorksheet->setCellValue($kolomkosong.$rowkosong,"=SUM(".$rowAwal.":".$rowAkhir.")");
		$objWorksheet->getStyle($kolomkosong.$rowkosong)->applyFromArray($styleT);
		...
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPexcel, 'Excel5');
		$objWriter->save('Templates/report/laporan_bkpp_rekap_eselon_jabatan.xls');

		$down= 'Templates/report/laporan_bkpp_rekap_eselon_jabatan.xls';
		header('Content-Description: File Transfer');
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment; filename='.basename($down));
		header('Content-Transfer-Encoding: binary');
	...
	function pensiun()
	{
		...
		$objPHPexcel= PHPExcel_IOFactory::load('Templates/report/pensiun.xlsx');
		$style = array(
			'borders' => array(
				'allborders' => array(
		...
		$this->load->model("base/Rekap");
		$set = new Rekap();
		$set->selectByParamsReportPensiunAll(array(), -1, -1, $statement, $reqPeriode, $reqTahun);	
		// $urut=1;
		$nomor=1;
		$kolomawal=1;
		$row = 10;
		...
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPexcel, 'Excel5');
		$objWriter->save('Templates/report/laporan_skpd_pensiun_excel.xls');

		$down= 'Templates/report/laporan_skpd_pensiun_excel.xls';
		header('Content-Description: File Transfer');
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment; filename='.basename($down));
		header('Content-Transfer-Encoding: binary');
