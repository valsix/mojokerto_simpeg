...
class penghargaan_json extends CI_Controller {
	...
	function add()
	{
		...
		$reqPENGHARGAAN_ID = $this->input->post("reqPENGHARGAAN_ID");
		...
		$penghargaan->setField('PEJABAT_PENETAP_ID', $reqTemp);	
		$penghargaan->setField('PEJABAT_PENETAP', strtoupper($reqPjPenetap));	

		$penghargaan->setField('NAMA', $reqNamaPenghargaan);
		$penghargaan->setField('TAHUN', $reqTahun);
		$penghargaan->setField('TANGGAL_SK', dateToDBCheck($reqTglSK));
		$penghargaan->setField('NO_SK', $reqNoSK);
		$penghargaan->setField('PEGAWAI_ID', $reqId);

		$penghargaan->setField('PENGHARGAAN_ID', $reqRowId);
		...
		if ($reqMode == "insert")
		{
			...
			$penghargaan->setField("LAST_CREATE_USER", $adminusernama);
			$penghargaan->setField("LAST_CREATE_DATE", "NOW()");	
			$penghargaan->setField("LAST_CREATE_SATKER", $userSatkerId);
	
			if($penghargaan->insert())
			{
				$reqSimpan= 1;
			}
		}
		...
		{	
			$penghargaan->setField("LAST_UPDATE_USER", $adminusernama);
			$penghargaan->setField("LAST_UPDATE_DATE", "NOW()");	
			$penghargaan->setField("LAST_UPDATE_SATKER", $userSatkerId);
			if($penghargaan->update())
			{
				$reqSimpan= 1;
			}
		}
