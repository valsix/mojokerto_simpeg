...
class cuti_json extends CI_Controller {
	...
	function add()
	{
		...
		if(strtotime($reqTanggalMulai) > strtotime($reqTanggalSelesai) )
		{
		...
		$cuti->setField('NO_SURAT', $reqNoSurat);
		$cuti->setField('JENIS_CUTI', $reqJenisCuti);
		$cuti->setField('TANGGAL_PERMOHONAN', dateToDBCheck($reqTanggalPermohonan));
		$cuti->setField('TANGGAL_SURAT', dateToDBCheck($reqTanggalSurat));
		$cuti->setField('TANGGAL_MULAI', dateToDBCheck($reqTanggalMulai));
		$cuti->setField('TANGGAL_SELESAI', dateToDBCheck($reqTanggalSelesai));	
		$cuti->setField('LAMA', $reqLama);
		$cuti->setField('KETERANGAN', $reqKeterangan);
		$cuti->setField('PEGAWAI_ID', $reqId);

		$cuti->setField('CUTI_ID', $reqRowId);

		$reqSimpan= "";
		if ($reqMode == "insert")
		{

			$cuti->setField("LAST_CREATE_USER", $adminusernama);
			$cuti->setField("LAST_CREATE_DATE", "NOW()");	
			$cuti->setField("LAST_CREATE_SATKER", $userSatkerId);
		...
		{	
			$cuti->setField("LAST_UPDATE_USER", $adminusernama);
			$cuti->setField("LAST_UPDATE_DATE", "NOW()");	
			$cuti->setField("LAST_UPDATE_SATKER", $userSatkerId);
			if($cuti->update())
			{
				$reqSimpan= 1;
			}
