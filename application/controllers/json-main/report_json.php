...
class report_json extends CI_Controller {
	...
	function json_skpd_semester()
	{
		...
		$sOrder = " ";
		$set->selectByParamsReportTribulanAll(array(), $displaylength, $displaystart, $statement,"");
		// echo $set->query;exit;
		while ($set->nextRow()) 
		{
			$row= [];
	...
	function json_rekap_eselon_kosong()
	{
		$this->load->model("base/Rekap");
		...
		// $sOrder = " ORDER BY A.TANGGAL ASC";
		$sOrder = " ";
		$set->selectByParamsBkppRekapEselonJabatanKosong(array(), $displaylength, $displaystart, $statement, $sOrder);
		// echo $set->query;exit;
		while ($set->nextRow()) 
		{
			$row= [];
	...
	function json_rekap_eselon_terisi()
	{
		...
		// $sOrder = " ORDER BY A.TANGGAL ASC";
		$sOrder = " ";
		$set->selectByParamsBkppRekapEselon(array(), $displaylength, $displaystart, $statement, $sOrder);
		// echo $set->query;exit;
		while ($set->nextRow()) 
		{
			$row= [];
	...
	function json_rekap_fungsional()
	{
		$this->load->model("base/Rekap");
		...
		// $sOrder = " ORDER BY A.TANGGAL ASC";
		$sOrder = " ";
		$set->selectByParamsBkppRekapFungsionalStaf(array(), $displaylength, $displaystart, $statement, $sOrder);
		// echo $set->query;exit;
		while ($set->nextRow()) 
		{
			$row= [];
	...
	function json_rekap_pensiun()
	{
		...
		{
			if($reqSatkerId == "")
				$statement = " AND SATKER_ID LIKE '".$reqSatkerId."%' ";
			else
				$statement = " AND SATKER_ID LIKE '".$reqSatkerId."%' ";
		}
		else // kondisi login sebagai SKPD
		{
			if($reqSatkerId == "")
				$statement = " AND SATKER_ID LIKE '".$this->adminsatkerid."%' ";
			else
				$statement = " AND SATKER_ID LIKE '".$reqSatkerId."%' ";
		}
		...
		// $sOrder = " ORDER BY A.TANGGAL ASC";
		$sOrder = " ";
		$set->selectByParamsReportPensiunAll(array(), $displaylength, $displaystart, $statement,  $reqPeriode, $reqTahun);
		// echo $set->query;exit;
		while ($set->nextRow()) 
		{
			$row= [];
