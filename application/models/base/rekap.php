...
class Rekap extends Entity{ 
    ...
    function selectByParamsBkppRekapEselonJabatan($paramsArray=array(),$limit=-1,$from=-1,$statement='')
	{
		$str = "
				
				SELECT A.SATKER_ID, A.NAMA, TOT21,  TOT22,  TOT31,  TOT32 ,
		TOT41,  TOT42 , TOT51,  TOT52, TOT
		FROM
		(
		SELECT A.SATKER_ID, A.NAMA, COUNT(TOT21.PEGAWAI_ID) AS TOT21,  COUNT(TOT22.PEGAWAI_ID) AS TOT22,  COUNT(TOT31.PEGAWAI_ID) AS TOT31,  COUNT(TOT32.PEGAWAI_ID) AS TOT32 ,
		COUNT(TOT41.PEGAWAI_ID) AS TOT41,  COUNT(TOT42.PEGAWAI_ID) AS TOT42 , COUNT(TOT51.PEGAWAI_ID) AS TOT51,  COUNT(TOT52.PEGAWAI_ID) AS TOT52, COUNT(TOT.PEGAWAI_ID) AS TOT
		FROM SATKER A
		LEFT JOIN PEGAWAI B ON B.SATKER_ID LIKE A.SATKER_ID || '%'
		LEFT JOIN
		(SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 21 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT21 ON TOT21.PEGAWAI_ID  = B.PEGAWAI_ID
		LEFT JOIN
		(SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 22 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT22 ON TOT22.PEGAWAI_ID = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 31 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT31 ON TOT31.PEGAWAI_ID  = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 32 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT32 ON TOT32.PEGAWAI_ID  = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 41 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT41 ON TOT41.PEGAWAI_ID  = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 42 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT42 ON TOT42.PEGAWAI_ID  = B.PEGAWAI_ID
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 51 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT51 ON TOT51.PEGAWAI_ID  = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 52 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT52 ON TOT52.PEGAWAI_ID  = B.PEGAWAI_ID    
		LEFT JOIN   
		(SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND X.PEGAWAI_ID IS NOT NULL  AND Y.ESELON_ID >= 21 AND Y.ESELON_ID <=42  AND X.STATUS_PEGAWAI IN(1,2)) TOT ON TOT.PEGAWAI_ID  = B.PEGAWAI_ID 
		WHERE 1=1 AND B.STATUS_PEGAWAI IN(1, 2) AND A.SATKER_ID = '01' AND LENGTH(B.SATKER_ID) <= 4
		GROUP BY A.SATKER_ID, A.NAMA
		UNION ALL
		SELECT A.SATKER_ID, A.NAMA, COUNT(TOT21.PEGAWAI_ID) AS TOT21,  COUNT(TOT22.PEGAWAI_ID) AS TOT22,  COUNT(TOT31.PEGAWAI_ID) AS TOT31,  COUNT(TOT32.PEGAWAI_ID) AS TOT32 ,
		...
		LEFT JOIN PEGAWAI B ON B.SATKER_ID LIKE A.SATKER_ID || '%'
		LEFT JOIN
		(SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 21 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT21 ON TOT21.PEGAWAI_ID  = B.PEGAWAI_ID  
		LEFT JOIN
		(SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 22 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT22 ON TOT22.PEGAWAI_ID = B.PEGAWAI_ID  
		LEFT JOIN
		 (SELECT X.PEGAWAI_ID FROM PEGAWAI X, JABATAN_TERAKHIR Y WHERE X.PEGAWAI_ID = Y.PEGAWAI_ID::bigint AND X.TIPE_PEGAWAI_ID = '11' AND Y.ESELON_ID = 31 AND X.PEGAWAI_ID IS NOT NULL AND X.STATUS_PEGAWAI IN(1,2)) TOT31 ON TOT31.PEGAWAI_ID  = B.PEGAWAI_ID  
    ...
    function selectByParamsBkppTotalRekapEselonJabatanKosong($paramsArray=array(),$limit=-1,$from=-1,$statement='')
	{
		$str = "
			SELECT 
				 SUM(CASE WHEN ESELON_ID = '21' THEN 1 ELSE 0 END ) ESELON21,
				 SUM(CASE WHEN ESELON_ID = '22' THEN 1 ELSE 0 END ) ESELON22,
				 SUM(CASE WHEN ESELON_ID = '31' THEN 1 ELSE 0 END ) ESELON31,
				 SUM(CASE WHEN ESELON_ID = '32' THEN 1 ELSE 0 END ) ESELON32,
				 SUM(CASE WHEN ESELON_ID = '41' THEN 1 ELSE 0 END ) ESELON41,
				 SUM(CASE WHEN ESELON_ID = '42' THEN 1 ELSE 0 END ) ESELON42,
				 SUM(CASE WHEN ESELON_ID = '51' THEN 1 ELSE 0 END ) ESELON51,
				 SUM(CASE WHEN ESELON_ID = '52' THEN 1 ELSE 0 END ) ESELON52
