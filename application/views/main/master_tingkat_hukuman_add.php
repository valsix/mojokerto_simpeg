...
else
{
	...
	$set->selectByParams(array('TINGKAT_HUKUMAN_ID'=>$reqId));
	// echo $set->query;exit;
	$set->firstRow();
	$reqId= $set->getField('TINGKAT_HUKUMAN_ID');
	
	$reqNama 			= $set->getField('TINGKAT_HUKUMAN');
			...
			<div class="d-flex align-items-baseline flex-wrap mr-5">
				<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
					<li class="breadcrumb-item text-muted">
						<a class="" href="app/index/master_tingkat_hukuman"><?=$pgtitle?></a>
					</li>
					<li class="breadcrumb-item text-muted">
            ...
            <form class="form" id="ktloginform" method="POST" enctype="multipart/form-data" autocomplete="off">
	        	<div class="card-body">
	        		...
	        		<div class="form-group row">
	        			...
	        			<div class="col-lg-6 col-sm-12">
	        				<select name="reqPeraturanId" class="form-control">
	        					<? 
	        					$tingkat_hukuman->selectByParamsStatus(array(),-1,-1);
	        					while($tingkat_hukuman->nextRow()){ ?>
	        						<option value="<?=$tingkat_hukuman->getField('PERATURAN_ID')?>" 
	        							<? if($reqPeraturanId == $tingkat_hukuman->getField("PERATURAN_ID")) { ?> selected="selected" <? } ?>>
	        							<?=$tingkat_hukuman->getField('NAMA')?>
	        						</option>
	        					<? } ?>        
	        				</select>
	        			</div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		var formSubmitUrl = "json-main/tingkat_hukuman_json/add";
					...
					success: function (response) {
                        ...
                        else
                        {
                            Swal.fire({
                                ...
                                document.location.href = "app/index/master_tingkat_hukuman";
