	        	...
	        	<div class="card-footer">
	        		<div class="row">
	        			<div class="col-lg-9">
	        				...
	        				{
	        				?>
	        					<a class="btn btn-success font-weight-bold mr-2" href="admin/loadUrl/admin/viewer?reqForm=jabatan&reqPegawaiJabatanFileId=<?=$reqPegawaiJabatanFileId?>" target="_blank">Download</a>
	        					<a class="btn btn-danger font-weight-bold mr-2" onclick="btndeletefile(<?=$reqPegawaiJabatanFileId?>,<?=$reqPegawaiId?>,<?=$reqRowId?>)">Hapus File</a>
	        				<?
	        				}
	        				?>
	        			</div>
...
<script type="text/javascript">
	...
	$("#ktunor").select2({
		...
		allowClear: true,
		ajax: {
			url: "json-data/combo_json/autocompleteunor",
			dataType: 'json',
			delay: 250,
			data: function(params) {
				return {
	...
	function btndeletefile(fileid,reqPegawaiId,reqRowId) {
		if(fileid !== "")
        {
            urlAjax= "json-data/personal_json/jsonpegawaijabatantipedeletefile?reqFileId="+fileid+"&reqPegawaiId="+reqPegawaiId+"&reqRowId="+reqRowId;
            swal.fire({
