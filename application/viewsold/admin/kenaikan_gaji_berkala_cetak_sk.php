...
if($tempSatkerInduk == "03" && $this->userlevel == "5" || $reqKgb=="guru" || $reqTipePegawai=="2102")
...
for($i=0; $i<count($field_template); $i++)
{
	...
	elseif($field_template[$i] == "REQJABATAN")
	{
		if($reqKgb=="pegawai")
		{
			...
			{
				$tempValue= $kgb->getField("JABATAN_SUB");			
			}
			else
			{
	...
	elseif($field_template[$i] == "REQSATKER") 
	{
		...
		if($reqKgb=="guru" || $reqTipePegawai=="2102")
		{
			$string= ucAddress($kgb->getField("SATKER_NAMA"))." ".$tempKabupatenLamongan;
		}
		else
		{
			$string= ucAddress($kgb->getField("SATKER"))." ".$tempKabupatenLamongan;
	...
	elseif($i == 9 || $i == 16){
		$tempValue= currencyToPage($kgb->getField($field[$i]),true);
		$rp= "Rp ".$tempValue;
		$document->setValue($field_template[$i], $rp);
	}
	elseif($i == 17){
		$arrayKGB = explode(' - ',$kgb->getField($field[$i]));
		$tmp = $arrayKGB[0];
