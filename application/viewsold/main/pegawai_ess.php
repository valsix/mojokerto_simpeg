...
$arrtabledata= array(
    ...
    ,array("label"=>"Nilai", "field"=> "Nilai", "display"=>"",  "width"=>"")
    ...
    , array("label"=>"fieldid", "field"=> "RIWAYAT_ESS_ID", "display"=>"1", "width"=>"")
);
...
jQuery(document).ready(function() {
    $("#reqSatuanKerja,#reqStatusPegawai,#reqBulan,#reqTipePegawai").change(function() {
      ...
      jsonurl= "json-data/pegawai_ess_json/json?reqTipePegawai="+reqTipePegawai+"&reqStatusPegawai="+reqStatusPegawai+"&reqSatuanKerja="+satuanKerja;
      // datanewtable.DataTable().filter.reqValidasi = 1;
      // datanewtable.DataTable().draw();
      datanewtable.DataTable().ajax.url(jsonurl).load();
    });

    var jsonurl= "json-data/pegawai_ess_json/json?reqBulan=<?=$reqBulan?>&reqTahun=<?=$reqTahun?>&reqValidasi=<?=$reqValidasi?>&reqSatuanKerja=<?=$reqSatuanKerja?>";
    ...
    $("#btnAdd,#btnUbahData").on("click", function () {
       btnid= $(this).attr('id');
        ...
        varurl= "app/index/pegawai_ess_data?formulaid=<?=$formulaid?>&reqId="+vpilihid;
...
$('#btnDelete').on('click', function (e) {
    ...
    {
        urlAjax= "json-data/pegawai_ess_json/delete?&reqDetilId="+valinfoid;
        swal.fire({
                ...
                $.ajax({
                    ...
                    success : function(data) { 
                        swal.fire({
                            ...
                            document.location.href = "app/index/pegawai_ess?formulaid=<?=$formulaid?>";
                        });
                    },
                    complete: function() {
                        swal.hideLoading();
