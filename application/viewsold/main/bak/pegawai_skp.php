...
$reqIdpenilaianprestasi = $this->input->get("reqIdpenilaianprestasi"); 
$arrdata= array(
    array("label"=>"Jenis Jabatan", "field"=> "JENIS_JABATAN_INFO", "display"=>"",  "width"=>"")
    , array("label"=>"Tahun", "field"=> "TAHUN", "display"=>"",  "width"=>"")
    , array("label"=>"Nilai SKP", "field"=> "SASARAN_KERJA", "display"=>"",  "width"=>"")
    , array("label"=>"Nilai Perilaku Kerja", "field"=> "PERILAKU_HASIL", "display"=>"",  "width"=>"")
    , array("label"=>"Nilai Prestasi Kerja ", "field"=> "NILAI_HASIL", "display"=>"",  "width"=>"")
    , array("label"=>"Pejabat Penilai", "field"=> "NAMA_PENILAI", "display"=>"",  "width"=>"")
    , array("label"=>"Atasan Pejabat Penilai", "field"=> "ATASAN_PENILAI", "display"=>"",  "width"=>"")
    ...
    , array("label"=>"fieldid", "field"=> "PENILAIAN_KERJA_PEGAWAI_ID", "display"=>"1", "width"=>"")
);
?>
...
<div style="height:0px;overflow:hidden">
    <form id="ktloginform" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" id="reqTahun" name="reqTahun" />
...
jQuery(document).ready(function() {
    var jsonurl= "json-validasi/personal_json/jsonpegawaiskp";
    ...
    $("#btnAdd").on("click", function () {
       var url = "app/index/pegawai_skp_add?reqPegawaiId=<?=$reqPegawaiId?>";
    ...
    $('#btnDelete').on('click', function (e) {
        ...
        {
            urlAjax= "json-validasi/personal_json/jsonpegawaiskpdelete?&reqDetilId="+valinfovalidasiid;
            swal.fire({
                    ...
                    $.ajax({
                        ...
                        success : function(data) { 
                            swal.fire({
                                ...
                                document.location.href = "app/index/pegawai_skp";
                            });
                        },
                        complete: function() {
                            swal.hideLoading();
    ...
    $("#btnUbahData").on("click", function () {
        ...
        {
           var url = "app/index/pegawai_skp_add?reqPegawaiId=<?=$reqPegawaiId?>&reqMode=edit&reqDetilId="+valinfovalidasiid+"&reqRowId="+valinfoid;
           window.location.href = url;
        }
...
function btnDeleteFile (fileid,reqPegawaiId,reqDetilId,reqRowId) {
    if(fileid !== "")
    {
        urlAjax= "json-validasi/personal_json/jsonpegawaiskpdeletefile?reqFileId="+fileid+"&reqPegawaiId="+reqPegawaiId+"&reqDetilId="+reqDetilId+"&reqRowId="+reqRowId;
        swal.fire({
...
function btnUpload(reqDetilId,reqPegawaiId,reqRowId,reqTahun)
{
    $("#reqLinkFile").click();
...
jQuery(document).ready(function(){
    jQuery('#ktloginform').submit(function(event){ 
        event.preventDefault();
           ...
           $.ajax({
            url: 'json-validasi/personal_json/jsonpegawaiskpupload', 
            dataType: 'json', 
            cache: false,
            contentType: false,
            processData: false,
